...
callback context, and has access to the Identity model (using `model`) and the request
object (using `req`).  Note  that model.auth_key defaults to 'email', but is also configurable.
